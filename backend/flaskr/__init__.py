...
def create_app(test_config=None):
  ...
  def add_question():
    try:
      ...
      if ( (question is None) or (question is None) or (question is None) or (question is None)):
        abort(400)
  ...
  def search_questions():
      try:
        ...
        if searchTerm:
          search_result = Question.query.filter(Question.question.ilike(f'%{searchTerm}%')).all()
          return jsonify({
            'success': True,
